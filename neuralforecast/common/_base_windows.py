...
class BaseWindows(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.lowmem_dataset:
            windows = batch["temporal"]
            original_outsample_y = torch.clone(windows[:, -self.h :, y_idx])
            insample_y = windows[:, : self.input_size, 0]
            insample_mask = windows[:, : self.input_size, 1]
            outsample_y = windows[:, self.input_size :, 0]
            outsample_mask = windows[:, self.input_size :, 1]
            hist_exog = None
            futr_exog = None
            stat_exog = None
        else:
    ...
    def validation_step_lowmem(self, batch, batch_idx):
        windows = batch["temporal"]
        y_idx = batch["y_idx"]
        original_outsample_y = torch.clone(windows[:, -self.h :, y_idx])
        ...
        valid_loss = self.valid_loss(
            ...
            y_hat=output_batch,
            mask=windows[:, self.input_size :, 1],
        )
    ...
    def validation_step(self, batch, batch_idx):
        if self.lowmem_dataset:
            return self.validation_step_lowmem(batch, batch_idx)
    ...
    def on_validation_epoch_end(self):
        if not self.lowmem_dataset and self.val_size == 0:
            return
    ...
    def predict_step_lowmem(self, batch, batch_idx):
        windows = batch["temporal"]
    ...
    def predict_step(self, batch, batch_idx):
        if self.lowmem_dataset:
            return self.predict_step_lowmem(batch, batch_idx)
    ...
    def fit(
        ...
        **Parameters:**<br>
        ...
        if self.lowmem_dataset:
            datamodule = LowMemTSDataModule(
                dataset,
                val_dataset,
                test_dataset,
                self.batch_size,
                self.num_workers_loader,
                self.drop_last_loader,
            )
        else:
